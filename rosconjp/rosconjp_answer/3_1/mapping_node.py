...
class MappingNode(Node):  # type: ignore
    ...
    def _update_map(self, scan_points: Points2d) -> None:
        ...
        estimated_bf_to_odom = self._odom_pose
        scan_points_on_map = estimated_bf_to_odom.transform(scan_points.points)
        for p in scan_points_on_map:
            self._accumulated_map.add(p)
        scan_points_on_map = self._odom_pose.transform(scan_points.points)
    ...
    def _scan_callback(self, scan_msg: LaserScan) -> None:
        ...
        # 相対的な移動変換を累積する
        self._odom_pose = self._odom_pose.multiply(odom)
